...
from application.models import (db, Guardians, NivelSeguranca, Specialization, EventoPontuacao, GuardianFeatured, User,
...
def guardian_hub():
    ...
    externos = User.query.filter_by(is_externo=True, is_active=True).order_by(User.name).all()
    quizzes = Quiz.query.filter_by(is_active=True).order_by(Quiz.title).all()
...
def analytics_hub():
    ...
    gsi_top5 = []
    externos = User.query.filter_by(is_externo=True, is_active=True).order_by(User.name).all()
    quizzes_ativos = Quiz.query.filter_by(is_active=True).order_by(Quiz.title).all()
...
@admin_v2_bp.route('/admin/externos/reset-temporada', methods=['POST'])
@guardian_admin_required
def reset_temporada_externa():
    try:
        externos = User.query.filter_by(is_externo=True).all()
        external_ids = [u.id for u in externos]

        # Debug: loga o que foi encontrado
        print(f"[RESET] Externos encontrados: {len(external_ids)} — IDs: {external_ids}")

        if not external_ids:
            flash('Nenhum usuário externo encontrado.', 'warning')
            return redirect(url_for('admin_v2_bp.guardian_hub'))

        # 1. Busca as tentativas antes de deletar
        attempts = QuizAttempt.query.filter(
            QuizAttempt.external_user_id.in_(external_ids)
        ).all()

        attempt_ids = [a.id for a in attempts]
        print(f"[RESET] Tentativas encontradas: {len(attempt_ids)}")

        # 2. Deleta UserAnswers manualmente (evita violação de FK)
        if attempt_ids:
            deleted_answers = UserAnswer.query.filter(
                UserAnswer.quiz_attempt_id.in_(attempt_ids)
            ).delete(synchronize_session=False)
            print(f"[RESET] UserAnswers deletadas: {deleted_answers}")

        # 3. Deleta as tentativas
        deleted_attempts = QuizAttempt.query.filter(
            QuizAttempt.external_user_id.in_(external_ids)
        ).delete(synchronize_session=False)
        print(f"[RESET] QuizAttempts deletadas: {deleted_attempts}")

        db.session.commit()
        flash(f'Temporada resetada! {deleted_attempts} tentativa(s) removida(s).', 'success')

    except Exception as e:
        db.session.rollback()
        print(f"[RESET] ERRO: {e}")
        flash(f'Erro ao resetar temporada: {e}', 'danger')

    return redirect(url_for('admin_v2_bp.guardian_hub'))

@admin_v2_bp.route('/admin/externos/reset-quiz', methods=['POST'])
@guardian_admin_required
def reset_quiz_externo():
    """
    Reseta a tentativa de um quiz específico de um usuário externo.
    """
    ...
    try:
        ...
        if not user.is_externo:
            flash('Operação permitida apenas para usuários externos.', 'warning')
            return redirect(url_for('admin_v2_bp.guardian_hub'))
        ...
        if not attempt:
            flash('Tentativa não encontrada.', 'warning')
            return redirect(url_for('admin_v2_bp.guardian_hub'))

        # Remove UserAnswers vinculadas antes
        UserAnswer.query.filter_by(quiz_attempt_id=attempt.id).delete()
        db.session.delete(attempt)
        db.session.commit()

        flash(f'Tentativa do quiz #{quiz_id} resetada para {user.name or user.username}.', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'Erro ao resetar tentativa: {e}', 'danger')
