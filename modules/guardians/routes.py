...
from modules.login.decorators import login_required, externo_required
...
from application.models import (MissionCodeEnum, db, TermoGame, Guardians, HistoricoAcao, NivelSeguranca, Insignia, GuardianFeatured, User,
...
def comprar_item(item_id):
    ...
    try:
        ...
        # Calcula expiração (None = permanente)
...
#LIDA COM USERS EXTERNOS
@guardians_bp.route('/guardians/central-externa')
@externo_required
def central_externa():
    """
    Central de desafios para usuários externos.
    Exibe apenas quizzes disponíveis, sem bônus nem perfil Guardian.
    """
    from modules.login.decorators import externo_required
    ...
    # Busca tentativas já feitas por esse user_id diretamente
    attempted_quiz_ids = db.session.query(QuizAttempt.quiz_id).filter(
        QuizAttempt.external_user_id == user_id
    )
    ...
    return render_template(
        'guardians/page_central_externa.html',
        available_activities=activities,
    )

@guardians_bp.route('/guardians/quiz-externo/start/<int:quiz_id>')
@externo_required
def start_quiz_externo(quiz_id):
    """
    Cria uma tentativa de quiz para usuário externo e redireciona para a sala de jogo.
    """
    user_id = SessionManager.get("user_id")
    quiz = Quiz.query.get_or_404(quiz_id)

    if not quiz.is_active:
        flash("Este quiz está desativado ou expirado.", "warning")
        return redirect(url_for('guardians_bp.central_externa'))

    # Verifica se já tentou
    ...
    if existing:
        if existing.completed_at:
            flash("Você já completou este quiz.", "info")
            return redirect(url_for('guardians_bp.central_externa'))
        # Tentativa incompleta: retoma
        return redirect(url_for('guardians_bp.take_quiz_externo', quiz_id=quiz_id))
...
@guardians_bp.route('/guardians/quiz-externo/play/<int:quiz_id>')
@externo_required
def take_quiz_externo(quiz_id):
    """
    Sala de jogo do quiz para usuário externo.
    Reaproveita o mesmo template do quiz interno.
    """
    user_id = SessionManager.get("user_id")
    quiz = Quiz.query.get_or_404(quiz_id)
    ...
    if quiz.time_limit_seconds and quiz.time_limit_seconds > 0:
        ...
        if start_time.tzinfo is None:
            start_time = start_time.replace(tzinfo=timezone.utc)
        elapsed = (datetime.now(timezone.utc) - start_time).total_seconds()
        remaining_time = int(max(0, quiz.time_limit_seconds - elapsed))
    ...
    return render_template(
        ...
        remaining_time=remaining_time,
        assistant_data=None,
        is_externo=True  # flag para o template esconder elementos internos se necessário
    )

@guardians_bp.route('/guardians/quiz-externo/submit', methods=['POST'])
@externo_required
def submit_quiz_externo():
    """
    Processa as respostas do quiz externo.
    Cálculo simplificado: apenas pontuação base, sem bônus.
    """
    try:
        user_id = SessionManager.get("user_id")
        quiz_id = request.form.get('quiz_id')
        ...
        if not attempt:
            flash("Tentativa não encontrada. Comece novamente.", "danger")
            return redirect(url_for('guardians_bp.central_externa'))
        ...
        if is_timer_expired or is_abandoned:
            message = "Tempo esgotado! Tentativa zerada." if is_timer_expired else "Quiz abandonado."
            attempt.score = 0
            attempt.final_score = 0
            attempt.completed_at = datetime.now(timezone.utc)
            db.session.commit()
        ...
        for question in quiz.questions:
            ...
            submitted_ids = [
                int(i) for i in request.form.getlist(f'question_{question.id}')
            ]
        ...
        attempt.final_score = raw_score
        attempt.completed_at = datetime.now(timezone.utc)

        # Zera bônus explicitamente (garantia)
        attempt.time_bonus = 0
        attempt.perfection_bonus = 0
        attempt.shop_bonus = 0
        attempt.conquista_bonus = 0
        attempt.streak_total_bonus = 0

        db.session.commit()

        return redirect(url_for(
            'guardians_bp.quiz_result_externo',
            quiz_id=quiz.id
        ))

    except Exception as e:
        db.session.rollback()
        flash('Erro ao processar o quiz. Tente novamente.', 'danger')
        print(f"ERRO submit_quiz_externo: {e}")
        return redirect(url_for('guardians_bp.central_externa'))
    
@guardians_bp.route('/guardians/quiz-externo/<int:quiz_id>/resultado')
@externo_required
def quiz_result_externo(quiz_id):
    """
    Página de resultado simplificada para usuário externo.
    Exibe apenas pontuação base e acertos/erros.
    """
    user_id = SessionManager.get("user_id")
    quiz = Quiz.query.get_or_404(quiz_id)
    ...
    # Reconstrói acertos para exibição
    user_answers = UserAnswer.query.filter_by(quiz_attempt_id=attempt.id).all()
    user_map = {}
    for ua in user_answers:
        user_map.setdefault(ua.question_id, []).append(ua.selected_option_id)

    correct_count = 0
    total_possible = 0
    for question in quiz.questions:
        total_possible += question.points
        user_ids = user_map.get(question.id, [])
        correct_ids = [o.id for o in question.options if o.is_correct]
    ...
    return render_template(
        'guardians/page_resultado_externo.html',
        quiz=quiz,
        attempt=attempt,
...
@guardians_bp.route('/guardians/ranking-externo')
@externo_required
def ranking_externo():
    """
    Ranking de pontuação exclusivo para usuários externos.
    Baseado no final_score acumulado das QuizAttempts.
    """
    # Busca todos os users externos
    externos = User.query.filter_by(is_externo=True, is_active=True).all()
    external_ids = [u.id for u in externos]

    # Agrega pontuação total por external_user_id
    scores = db.session.query(
        QuizAttempt.external_user_id,
        db.func.sum(QuizAttempt.final_score).label('total_score'),
        db.func.count(QuizAttempt.id).label('quizzes_completed')
    ).filter(
        QuizAttempt.external_user_id.in_(external_ids),
        QuizAttempt.completed_at.isnot(None)
    ).group_by(QuizAttempt.external_user_id).all()

    # Monta dicionário user_id -> dados
    user_map = {u.id: u for u in externos}

    ranking = []
    for row in scores:
        user = user_map.get(row.external_user_id)
        if user:
            ranking.append({
                'name': user.name or user.username,
                'avatar_seed': user.avatar_seed or 'GuardianDefault',  # NOVO
                'total_score': row.total_score or 0,
                'quizzes_completed': row.quizzes_completed
            })

    # Usuários que ainda não completaram nenhum quiz também aparecem com 0
    scored_ids = {row.external_user_id for row in scores}
    for user in externos:
        if user.id not in scored_ids:
            ranking.append({
                'name': user.name or user.username,
                'avatar_seed': user.avatar_seed or 'GuardianDefault',  # NOVO
                'total_score': 0,
    ...
    return render_template(
        'guardians/page_ranking_externo.html',
        ranking=ranking
    )

@guardians_bp.route('/guardians/meu-perfil-externo')
@externo_required
def meu_perfil_externo():
    """
    Página de perfil simplificada para usuários externos.
    Sem Guardian, sem bônus, sem inventário.
    """
    user_id = SessionManager.get("user_id")
    ...
    # --- Estatísticas ---
    total_score = sum(a.final_score or 0 for a in attempts)
    quizzes_completed = len(attempts)

    # Acertos totais (recalcula via UserAnswers)
    total_correct = 0
    total_questions = 0
    for attempt in attempts:
        answers = UserAnswer.query.filter_by(quiz_attempt_id=attempt.id).all()
        user_map = {}
        for ua in answers:
            user_map.setdefault(ua.question_id, []).append(ua.selected_option_id)
    ...
    all_scores = db.session.query(
        QuizAttempt.external_user_id,
        db.func.sum(QuizAttempt.final_score).label('total')
    ).filter(
        QuizAttempt.completed_at.isnot(None),
        QuizAttempt.external_user_id.isnot(None)
    ).group_by(QuizAttempt.external_user_id)\
     .order_by(db.text('total DESC')).all()

    ranking_pos = 'N/A'
    for i, row in enumerate(all_scores):
        if row.external_user_id == user_id:
            ranking_pos = i + 1
            break

    # --- Histórico por quiz (para exibição) ---
    historico = []
    for attempt in attempts:
        historico.append({
            'quiz_title': attempt.quiz.title if attempt.quiz else 'Quiz removido',
            'final_score': attempt.final_score or 0,
            'completed_at': attempt.completed_at,
        })

    return render_template(
        'guardians/page_perfil_externo.html',
        user=user,
        avatar_seed=user.avatar_seed or 'GuardianDefault',
...
@guardians_bp.route('/guardians/perfil-externo/editar', methods=['POST'])
@externo_required
def edit_perfil_externo():
    user_id = SessionManager.get("user_id")
    user = User.query.get_or_404(user_id)

    novo_nome = request.form.get('display_name', '').strip()
    novo_avatar = request.form.get('avatar_seed', '').strip()

    if novo_nome and len(novo_nome) <= 50:
        user.name = novo_nome

    if novo_avatar:
        user.avatar_seed = novo_avatar
    ...
    return redirect(url_for('guardians_bp.meu_perfil_externo'))